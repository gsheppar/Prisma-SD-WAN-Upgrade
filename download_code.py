...
def download(cgx, lists_from_csv, image):
    ...
    if image not in image_options:
        ...
        return
    image_download = image_n2id[image]
    ...
    if len(element_list) != 0:
        ...
        for element_id in element_list:
            ...
            data = {"_etag":resp['_etag'],"_schema":resp['_schema'],"image_id":image_download,"scheduled_upgrade":None,"scheduled_download":time_now,"download_interval":None,"upgrade_interval":None,"interface_ids":None}
            resp = cgx.put.software_state(element_id=element_id, data=data).cgx_content
            if not resp:
                print(str(jdout(resp)))
                element_list.remove(element_id)
            else:
                print("Downloading " + image + " on " + element_id2n[element_id])
    ...
    if len(element_list) != 0:
        ...
        while time_check < 1800:
            ...
            for element_id in check_element_list:
                ...
                if image_id2n[status['upgrade_image_id']] == image:
                    if status['download_percent'] == 100:
                        print(element_id2n[element_id] + " has finished its download of " + image)
                        element_list.remove(element_id)
        ...
        if len(element_list) != 0:
            for element_id in element_list:
                print("Downloading " + image + " on " + element_id2n[element_id] + " did not complete in time please check the ION.")
